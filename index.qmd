...
High altitude reduces air density, causing baseballs to travel farther with less air resistance and making pitches easier to hit. Coors Field in Denver sits at 5,200 feet above sea level - the highest elevation in MLB - creating the well-documented "Coors Effect" where balls travel 5-10% farther, resulting in significantly more runs scored per game compared to sea-level stadiums.
...
print("2010 games:", len(df2010))
print("2021 games:", len(df2021))

# Show why we aggregate - individual games vary too much
print("\nSample individual games:")
print(df2010[['home', 'homeScore', 'visScore']].head(5))

print("\nStadium averages (much cleaner):")
...
A horizontal bar chart is better than a scatter plot because we're comparing categorical data (stadiums) rather than continuous variables, and it makes stadium names easier to read. You would choose a vertical bar chart over horizontal when you have fewer categories (5-10 items) or when stadium names are short enough to fit without rotation.
...
The color highlighting in Approach 4 draws immediate attention to Colorado (COL) as the standout performer, making it easy to identify the highest-scoring stadium at a glance. Position (x/y coordinates) is better for encoding precise quantitative values, while color is better for highlighting specific categories or drawing attention to particular data points.
...
The visualization is suitable for business presentations because it has a clear, bold title that states the main finding, professional color scheme with strategic highlighting, clean grid lines for easy reading, and a legend that explains the color coding. The annotation is helpful but could be improved by placing it closer to the Colorado bar.
...
ax2.set_title("2021: Colorado Maintains Dominance", fontsize=14, fontweight='bold', pad=20)
...
for ax, data in [(ax1, avgDF_2010_sorted), (ax2, avgDF_2021_sorted)]:
    if "COL" in data.home.values:
        ...
        ax.text(colorado_runs + max(data.totalRuns) * 0.01,  # small offset proportional to axis
                "COL", f"{colorado_runs:.2f}",
                fontsize=9, fontweight='bold', color='darkorchid',
                ha='left', va='center')

# Add overall title
fig.suptitle("Coors Field: Consistently the Most Run-Friendly Ballpark", 
...
This visualization displays average total runs scored per game at each Major League Baseball stadium for the 2010 and 2021 seasons, placing the two years side-by-side for easy comparison. Each bar represents a team's home stadium, with Coors Field (home of the Colorado Rockies, "COL") uniquely highlighted in purple. By separating the seasons into two horizontal bar charts, the figure allows the viewer to assess both the general distribution of scoring environments across stadiums and the exceptional offensiveness of Coors Field. The fact that Colorado's bar is noticeably longer than those of other stadiums in both years, and clearly stands out in color, visually confirms that Coors Field is indeed the most run-friendly ballpark in Major League Baseball for both seasons analyzed. This supports the hypothesis about high altitude influencing game outcomes and provides a clear, data-driven narrative for business or sports analysis audiences.


This is incorrect, Colarado is not the most run-friendly ballpark in 2021. The visualization is correct, but the conclusion is incorrect.
:::
